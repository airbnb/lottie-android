...
* Lottie can now render outside of its composition bounds. To allow this with views such as LottieAnimationView, set `clipChildren` to false on the parent view. For Compose, this is the default behavior but it can be disabled with the `clipToBounds()` modifier.
