...
sealed class LottieClipSpec {
    ...
    data class Markers(
        ...
        override fun getMinProgress(composition: LottieComposition): Float {
            return when (min) {
                null -> 0f
                else -> ((composition.marker(min)?.startFrame ?: 0f) / composition.lastFrame).coerceIn(0f, 1f)
            }
        }
        ...
        override fun getMaxProgress(composition: LottieComposition): Float {
            return when (max) {
                null -> 1f
                    ...
                    return ((composition.marker(max)?.startFrame?.plus(offset) ?: 0f) / composition.lastFrame).coerceIn(0f, 1f)
                }
            }
        }
    }
    ...
    data class Marker(val marker: String) : LottieClipSpec() {
        override fun getMinProgress(composition: LottieComposition): Float {
            return ((composition.marker(marker)?.startFrame ?: 0f) / composition.lastFrame).coerceIn(0f, 1f)
        }
